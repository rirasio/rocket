...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/users/admin/**").hasRole("9")// 관리자
		...
		http.logout().logoutUrl("/users/logout")// 로그아웃을 요청할 경로이고 기본값은 /logout
				.invalidateHttpSession(true).and() // 403 예외처리 핸들링
				.exceptionHandling().accessDeniedPage("/users/access_denied");// 로그인 실패
		// http.logoutRequestMatcher(new AntPathRequestMatcher("/users/logout")) //로그아웃의
		// 기본 URL(/logout) 이 아닌 다른 URL로 재정의합니다.
		// http.logoutSuccessUrl("/users/logout")// 로그아웃이 처리된 후 이동될 경로이고 >>
		// 기본값은/login?logout >> 첫페이지 보낼때 사용할 수 있음
...
//		.authoritiesByUsernameQuery(queryDetails);
		...
		// test용 임시 계정
//		String pw = new BCryptPasswordEncoder().encode("1111");
//		auth.inMemoryAuthentication().withUser("1111").password(pw).roles("9");
